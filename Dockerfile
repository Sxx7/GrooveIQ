...
# Numba JIT cache (used by umap-learn). The compose file marks the rootfs
# read-only, so point at the writable /tmp tmpfs. The user has no $HOME
# either, so without this override numba can't cache and umap import
